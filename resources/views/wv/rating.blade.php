...
<body>
                    ...
                    <div style="margin-top: 0px">
                        @forelse ($ratings as $rating)
                            <div class="rating-item"
                                style="border-bottom: 1px solid #b3b3b3; padding-bottom: 5px; padding-top: 20px">
                                <div>
                                    @for ($i = 0; $i < $rating->rating; $i++)
                                        <img src="{{ url('assets/star.svg') }}" style="width: 15px" alt="">
                                    @endfor
                                    @for ($i = 0; $i < 5 - $rating->rating; $i++)
                                        <img src="{{ url('assets/star-grey.svg') }}" style="width: 15px" alt="">
                                    @endfor

                                    <span
                                        style="float: right; color: #333">{{ date('d-m-Y', strtotime($rating->created_at)) }}</span>
                                </div>
                        ...
                        @endforelse
                        <button data-toggle="modal" data-target="#ratingModal" style="margin-top: 30px"
                            class="btn btn-block btn-primary">Beri Rating</button>
                    </div>
                </div>
    ...
    <div class="modal fade" id="ratingModal" tabindex="-1" role="dialog" aria-labelledby="ratingModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <form method="post" action="{{ route('wv.rating-post', $data->id_konten) }}" onsubmit="return cek()" class="modal-content">
                @csrf
                ...
                <div class="modal-body">
                    <div class="feedback">
                        <div class="rating">
                            <input type="radio" name="rating" class="rating-star" rating-value="5" id="rating-5">
                            <label for="rating-5"></label>
                            <input type="radio" name="rating" class="rating-star" rating-value="4" id="rating-4">
                            <label for="rating-4"></label>
                            <input type="radio" name="rating" class="rating-star" rating-value="3" id="rating-3">
                            <label for="rating-3"></label>
                            <input type="radio" name="rating" class="rating-star" rating-value="2" id="rating-2">
                            <label for="rating-2"></label>
                            <input type="radio" name="rating" class="rating-star" rating-value="1" id="rating-1">
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea style="height: 100px" name="komentar" id="" cols="30" rows="10" class="form-control" required></textarea>
                        </div>
                    </div>
    ...
    <script>
        $('.rating-star').click(function() {
            $('#rating-value').val($(this).attr('rating-value'));
        })

        function cek() {
            if($('#rating-value').val() == '') {
                $('#error').html('Harap pilih rating terlebih dahulu');
                return false;
            }else{
                return true;
            }
